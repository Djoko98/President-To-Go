...
export default async function Page() {
  ...
  const { data } = await supabase.from("categories").select("id,name,slug,group_key,is_active,position").order("position");
  const input = "min-h-11 w-full rounded-xl border border-neutral-200 px-3";

  return <main className="p-5 sm:p-8">
    <p className="text-sm font-semibold text-neutral-500">Redosled, grupa i vidljivost</p>
    <h1 className="text-3xl font-bold tracking-[-.05em] sm:text-4xl">Kategorije</h1>
    <p className="mt-2 max-w-2xl text-sm text-neutral-500">Grupa određuje na kom delu točka se kategorija prikazuje gostima — piće ide levo, hrana desno od razdvojnika.</p>
    <div className="mt-6 grid gap-3 sm:mt-7 sm:gap-4">
      {(data ?? []).map((category) => <form key={category.id} action={saveCategory} className="grid gap-4 rounded-3xl bg-white p-4 sm:grid-cols-[50px_1fr_1fr_140px_auto] sm:items-end sm:p-5">
        ...
        <div><label className="text-sm font-bold">Grupa</label><select name="groupKey" defaultValue={category.group_key ?? "drinks"} className={`mt-2 ${input}`}>{CATEGORY_GROUPS.map((group) => <option key={group.key} value={group.key}>{group.label}</option>)}</select></div>
