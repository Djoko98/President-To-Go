...
export const getCatalog = cache(async (): Promise<CatalogData> => {
  try {
    ...
    const [{ data: categories, error: categoriesError }, { data: products, error: productsError }, { data: settings }] = await Promise.all([
      ...
      supabase.from("products").select("id,category_id,name,slug,description,ingredients,price,image_url,accent_color,contains_alcohol,preparation_minutes,max_quantity_per_order,position,is_available,is_active").eq("is_active", true).order("position").order("created_at"),
