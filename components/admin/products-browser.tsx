...
function ProductCard({ product, first, last, canManage }: { product: ProductRow; first: boolean; last: boolean; canManage: boolean }) {
  return (
  ...
  return (
    <>
      ...
      {active === "all" ? (
                ...
                <div className="mt-4 space-y-8">
                  {groupCategories.map((category) => (
                    <section key={category.id}>
                      ...
                      <div className="mt-3 grid gap-3 sm:gap-4 xl:grid-cols-2">{products.filter((product) => product.category_id === category.id).map((product) => <ProductCard key={product.id} product={product} canManage={canManage} {...edgesOf(product.id)} />)}</div>
                    </section>
                  ))}
                </div>
              </section>
        ...
        <div className="mt-6 grid gap-3 sm:mt-7 sm:gap-4 xl:grid-cols-2">{products.filter((product) => product.category_id === active).map((product) => <ProductCard key={product.id} product={product} canManage={canManage} {...edgesOf(product.id)} />)}</div>
      )}
