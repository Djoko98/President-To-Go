...
export function FloatingAddBar({ quantity, max, disabled, paused, onChange, onAdd }: { quantity: number; max: number; disabled?: boolean; paused?: boolean; onChange: (value: number) => void; onAdd: () => void }) {
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 px-3 pb-[max(12px,env(safe-area-inset-bottom))] sm:px-6">
      <div className="mx-auto flex w-full max-w-[760px] items-stretch gap-2 rounded-[32px] bg-[#171817] p-2.5 shadow-[0_18px_60px_rgba(0,0,0,.3)] sm:rounded-[36px] sm:p-3">
        {paused ? (
          <p role="status" className="flex min-h-14 w-full items-center justify-center px-5 text-center font-bold text-white sm:text-lg">Online poručivanje je trenutno pauzirano</p>
        ) : (
        <>
