...
export function CategorySelector({ categories, activeId, onChange }: { categories: Category[]; activeId: string; onChange: (category: Category) => void }) {
  return (
    <nav aria-label="Kategorije napitaka" className="catalog-categories relative mx-auto w-full max-w-[620px] overflow-hidden">
      <div aria-hidden className="absolute left-1/2 top-[62%] h-[240px] w-[130%] -translate-x-1/2 rounded-[50%] border-t border-white/90 shadow-[0_-1px_0_rgba(0,0,0,.035)]" />
      <div className="relative z-10 grid grid-cols-3 items-start px-2">
        {categories.map((category, index) => {
          const active = category.id === activeId;
          const lift = index === 1 ? "-translate-y-1" : "translate-y-3";
          return (
            <button key={category.id} type="button" aria-pressed={active} onClick={() => onChange(category)} className={`touch-target group flex flex-col items-center gap-1 rounded-3xl py-2 transition ${lift}`}>
              <span className={`catalog-category-image relative block rounded-full transition ${active ? "bg-white/80 shadow-[0_10px_35px_rgba(0,0,0,.06)]" : ""}`}>
