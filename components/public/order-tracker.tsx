...
export function OrderTracker({ token, initialOrder, phone }: { token: string; initialOrder: PublicOrder; phone: string }) {
  ...
  return (
    <main className="mx-auto max-w-2xl px-5 pb-16 pt-8 sm:px-8">
      ...
      <section className="mt-8 rounded-3xl bg-white p-5 shadow-sm"><div className="flex justify-between"><div><p className="text-sm text-neutral-500">{order.confirmed_pickup_at ? "Potvrđeno preuzimanje" : "Preuzimanje"}</p><p className="font-bold">{formatBelgradeTime(order.confirmed_pickup_at ?? order.requested_pickup_at)}</p></div><div className="text-right"><p className="text-sm text-neutral-500">Ukupno</p><p className="font-bold">{formatMoney(order.total)}</p></div></div><div className="mt-5 border-t border-neutral-100 pt-4">{order.items.map((item) => <div key={`${item.name}-${item.quantity}`} className="flex justify-between py-1"><span>{item.quantity} × {item.name}</span><span>{formatMoney(item.line_total)}</span></div>)}</div><p className="mt-4 text-sm text-neutral-500">Za: {order.customer_name} · telefon završava na {order.phone_last_four}</p></section>
